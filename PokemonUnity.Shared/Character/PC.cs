...
{	
	...
	public class PC
	{
		...
		public bool Add_PC_Pokemon(int Box, int BoxID, Pokemon pokemon)
        {
            try
            {
				pokemons[Box, BoxID] = pokemon;
				return true;
            }
            catch
            {
		...
		/// <summary>
		/// Add a new pokemon directly to box that has space. 
		...
		{
			//This run every box to get free space for new Pokemon
			for (byte box = 0; box < Core.STORAGEBOXES; box++)
			{
				if (this[box].hasSpace()) //Check if each box has space
				{
					Pokemons[getIndexOfFirstEmpty().Value] = acquiredPokemon;
					pokemons[box, getIndexOfFirstEmpty().Value] = acquiredPokemon;
					return true;
				}
			}
